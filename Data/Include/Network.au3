...
;Fonction ajouté
Func _GetNetworkAdapterStatus()

	;Déclaration de l'objet
	Local $objWMIService = ObjGet("winmgmts:\\" & @ComputerName & "\root\CIMV2")
	If @error Or $objWMIService = 0 Then Return 0

	;Déclaration de variable local
	Local $n = 0, $aResult[1][2] ;Tableau
	Local $colNetAdapter, $objNetAdapter

	;Filtre sur les cartes réseaux avec un NetConnectionID non null (connexion réseaux local, etc)
	$colNetAdapter = $objWMIService.ExecQuery('SELECT * FROM Win32_NetworkAdapter WHERE NetConnectionID IS NOT NULL', "WQL", $wbemFlagReturnImmediately + $wbemFlagForwardOnly)
	If IsObj($colNetAdapter) Then

		;Pour chaque objet NetworkAdapter récuperer le NetConnectionStatus
		For $objNetAdapter In $colNetAdapter
			$n += 1
			Redim $aResult[$n][2] ;Redimensionne le tableau
			$aResult[$n - 1][0] = $objNetAdapter.NetConnectionID ;Nom de la connexion
			$aResult[$n - 1][1] = _TradStatus($objNetAdapter.NetConnectionStatus) ;Etat de la connexion
		Next
	EndIf

	;Retourne le tableau ou 0 si vide
	If $n = 0 Then Return 0
	_ArraySort($aResult)
	Return $aResult
EndFunc ; ===> _GetNetworkAdapterStatus

;Fonction ajouté
Func _GetNetworkAdapterInfosModif()
	...
	;Déclaration de variable local
	Local $n = 0, $aResult[1][16] ;Tableau
	Local $colNetAdapter, $colNetAdapterConfig, $objNetAdapter, $objNetAdapterConfig

	;Filtre sur les cartes réseaux avec un NetConnectionID non null (connexion réseaux local, etc)
	$colNetAdapter = $objWMIService.ExecQuery('SELECT * FROM Win32_NetworkAdapter WHERE NetConnectionID IS NOT NULL', "WQL", $wbemFlagReturnImmediately + $wbemFlagForwardOnly)
	If Not IsObj($colNetAdapter) Then Return 0

	;Pour chaque objet NetworkAdapter
	For $objNetAdapter In $colNetAdapter

		;Récupéré un objet de type NetworkAdapterConfiguration (focus Index)
		$colNetAdapterConfig = $objWMIService.ExecQuery("SELECT * FROM Win32_NetworkAdapterConfiguration WHERE Index = '" & $objNetAdapter.Index & "'", "WQL", $wbemFlagReturnImmediately + $wbemFlagForwardOnly)
		If IsObj($colNetAdapterConfig) Then
			For $objNetAdapterConfig In $colNetAdapterConfig
				$n += 1
				Redim $aResult[$n][16] ;Redimensionne le tableau

				;Récupérer ces informations
				$aResult[$n - 1][0] = _ArrayReplaceEmpty($objNetAdapter.NetConnectionID) ;Nom de la connexion
				$aResult[$n - 1][1] = _ArrayReplaceEmpty($objNetAdapter.Manufacturer) ;Fabriquant
				$aResult[$n - 1][2] = _ArrayReplaceEmpty($objNetAdapter.Name) ;Modèle
				$aResult[$n - 1][3] = _ArrayReplaceEmpty(_TradStatus($objNetAdapter.NetConnectionStatus)) ;Etat de la connexion
				$aResult[$n - 1][4] = _ArrayReplaceEmpty($objNetAdapter.MACAddress) ;Adresse MAC
				$aResult[$n - 1][5] = _ArrayReplaceEmpty($objNetAdapter.AdapterType) ;Type de connexion (càble, wireless)
				$aResult[$n - 1][6] = _ArrayReplaceEmpty(_TradENtoFR($objNetAdapterConfig.DHCPEnabled)) ;DHCP activé (oui/non) - IPV4
				$aResult[$n - 1][7] = _ArrayReplaceEmpty($objNetAdapterConfig.DHCPServer) ;@IP du serveur DHCP - IPV4
				$aResult[$n - 1][8] = _ArrayReplaceEmpty(_WMIDate($objNetAdapterConfig.DHCPLeaseObtained, 1)) ;Date et heure du bail DHCP obtenu - IPV4
				$aResult[$n - 1][9] = _ArrayReplaceEmpty(_WMIDate($objNetAdapterConfig.DHCPLeaseExpires, 1)) ;Date et heure de l'expiration du bail DHCP - IPV4
				$aResult[$n - 1][10] = _ArrayReplaceEmpty(_Array2StringIP($objNetAdapterConfig.IPAddress, " | ")) ;@IP - IPV4/IPV6
				$aResult[$n - 1][11] = _ArrayReplaceEmpty(_Array2StringIP($objNetAdapterConfig.IPSubnet, " | ")) ;Masque - IPV4/IPV6
				$aResult[$n - 1][12] = _ArrayReplaceEmpty(_Array2StringIP($objNetAdapterConfig.DefaultIPGateway, " | ")) ;Passerelle - IPV4/IPV6
				$aResult[$n - 1][13] = _ArrayReplaceEmpty(_Array2StringIP($objNetAdapterConfig.DNSServerSearchOrder, " | ")) ;DNS Primaire / Secondaire - IPV4
				$aResult[$n - 1][14] = _ArrayReplaceEmpty(_Array2String($objNetAdapterConfig.DNSDomainSuffixSearchOrder, " | ")) ;Listes des suffixe DNS recherché
				$aResult[$n - 1][15] = _ArrayReplaceEmpty($objNetAdapterConfig.DNSHostName) ;Nom DNS du PC
			Next
		EndIf
	Next
...
EndFunc ; ===> _GetNetworkAdapterInfosModif
...
;Fonction ajouté
Func _Array2StringIP($avArray, $sDelim = " | ", $iStart = 0, $iEnd = 0)
	If Not IsArray($avArray) Then Return SetError(1, 0, "")
	If UBound($avArray, 0) <> 1 Then Return SetError(3, 0, "")
	Local $sResult, $iUBound = UBound($avArray) - 1
	...
	; Combine (only IPv4)
	For $i = $iStart To $iEnd
		If StringRegExp($avArray[$i], "(\d{1,3}\.)") Then $sResult &= $avArray[$i] & $sDelim
	Next

	Return StringTrimRight($sResult, StringLen($sDelim))
EndFunc   ;==>_Array2StringIP

;Fonction ajouté
Func _TradENtoFR($String)
	$String = StringReplace($String, "True", "Oui")
	$String = StringReplace($String, "False", "Non")
	Return $String
EndFunc

;Fonction ajouté
Func _TradStatus($number)
	Select
		Case $number = 0
			$number = "Déconnecté" ;Bouton couleur noire

		Case $number = 1
			$number = "Connexion en cours" ;Bouton couleur orange

		Case $number = 2
			$number = "Connecté" ;Bouton couleur vert

		Case $number = 3
			$number = "Déconnexion en cours" ;Bouton couleur orange

		Case $number = 4
			$number = "Carte désactivé" ;Bouton couleur noir

		Case $number = 5
			$number = "Matériel désactivé" ;Bouton couleur noir

		Case $number = 6
			$number = "Dysfonctionnement matériel" ;Bouton couleur noir

		Case $number = 7
			$number = "Média déconnecté" ;Bouton couleur orange

		Case $number = 8
			$number = "Authentification" ;Bouton couleur orange

		Case $number = 9
			$number = "Authentification réussie" ;Bouton couleur orange

		Case $number = 10
			$number = "Authentification échouée" ;Bouton couleur orange

		Case $number = 11
			$number = "Adresse invalide" ;Bouton couleur orange

		Case $number = 12
			$number = "Informations requises" ;Bouton couleur orange

		Case Else
			$number = "N/A" ;Bouton couleur noir
	EndSelect
...
;Fonction ajouté
Func _ArrayReplaceEmpty($String, $sReplace="N/A")
	If not $String Then $String = $sReplace
	Return $String
EndFunc